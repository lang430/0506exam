...
| Worker 配置 | Vercel Serverless 三路径互补：上传接口 after() 触发 `/api/import-dispatcher`（主路径）+ 调度端点 after() 自链续跑（30 批/50s 一轮）+ 进度轮询响应前内联调度（4 批/8s）；cron 与本地 worker-loop 兜底；全局租约锁（dispatch_lease）保证同一时刻单处理器串行消费 |
...
4. **已知瓶颈 3**：Vercel Hobby 函数 maxDuration 60s，单轮调度预算受其约束（调度端点 50s、轮询内联 8s）；通过"上传 after() 触发 + 调度端点自链 + 轮询自愈 + cron 兜底"四重路径保证任务最终完成。
