...
| 指标 | 结果 | 目标 | 结论 |
|---|---:|---|---|
| 10,000 行全链路耗时（上传返回 → 任务终态） | **5.6 秒**（其中上传接口 1.2 秒为压测机跨洋视角，服务端处理 150~260ms） | ≤ 60 秒 | ✅ 达标（余量 10 倍） |
...
1. **结论**：10,000 行全链路 5.6 秒（≤60s 目标的 9%），无 5xx，错误数与预埋完全一致，行级错误可定位、可导出、可检索。10,000 单/分钟目标达成，实测吞吐约 107,000 行/分钟。
