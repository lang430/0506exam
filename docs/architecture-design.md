...
| 批量处理策略 | 默认 500 行/批（`V4_BATCH_SIZE`），生产压测设为 2500 行/批；末批采用开放区间 `[start, -1)`；成功行以 250 行/块多行 UPSERT | 批次自包含可独立重试；大批次减少调度/认领开销（压测 5 批 38.5s），默认小批次降低单批被 10s 硬上限 kill 的风险 |
